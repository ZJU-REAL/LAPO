...
def update_samples_with_rewards(rewards_info, samples_list):
    ...
    acc_list = torch.cat([info["acc_rewards"] for info in rewards_info], dim=0).chunk(len(samples_list))
    format_list = torch.cat([info["format_rewards"] for info in rewards_info], dim=0).chunk(len(samples_list))
    # lengths_list = torch.cat([info["lengths"] for info in rewards_info], dim=0)
    # prompts_list = sum([s.prompts for s in samples_list], [])
    #
    # all_prompts = prompts_list
    # # print(lengths_list)
    # # print(len(prompts_list))
    # assert len(all_prompts) == len(lengths_list)
    #
    # prompt_to_lengths = defaultdict(list)
    #
    # for p in set(all_prompts):
    #     prompt_to_lengths[p] = []
    #
    # for p, l in zip(all_prompts, lengths_list.tolist()):
    #     if l > 0:
    #         prompt_to_lengths[p].append(l)
    #
    # median_path = "/home/xy/median_length-1.json"
    # mean_path = "/home/xy/mean_length-1.json"
    #
    # for prompt, all_lengths in prompt_to_lengths.items():
    #     median_length = int(np.median(all_lengths)) if all_lengths else 0
    #     mean_length = int(np.mean(all_lengths)) if all_lengths else 0
    #
    #     # 写 median
    #     with open(median_path, "a", encoding="utf-8") as f:
    #         f.write(json.dumps({"prompt": prompt, "median_length": median_length}, ensure_ascii=False) + "\n")
    #
    #     # 写 mean
    #     with open(mean_path, "a", encoding="utf-8") as f:
    #         f.write(json.dumps({"prompt": prompt, "mean_length": mean_length}, ensure_ascii=False) + "\n")
    #
    # # 4. 求每题的中位数和合理区间
    # prompt_to_leninfo = {}
    # for p, ls in prompt_to_lengths.items():
    #     if ls:
    #         lower_q = np.percentile(ls, 30)
    #         upper_q = np.percentile(ls, 70)
    #         prompt_to_leninfo[p] = (lower_q, upper_q)
    #     else:
    #         prompt_to_leninfo[p] = (0, 0)
    #
    # len_reward_vec = torch.zeros_like(lengths_list, dtype=torch.float32)
    # for idx, (p, l) in enumerate(zip(all_prompts, lengths_list.tolist())):
    #     lower_q, upper_q = prompt_to_leninfo[p]
    #     if lower_q < upper_q and l > 0:
    #         if lower_q <= l <= upper_q:
    #             len_reward = 1.0
    #         else:
    #             margin = min(abs(l - lower_q), abs(l - upper_q))
    #             len_reward = max(0.0, 1.0 - margin / 100.0)
    #     else:
    #         len_reward = 0.0
    #     len_reward_vec[idx] = len_reward
    #
    # num_blocks = len(samples_list)
    # len_reward_chunks = len_reward_vec.chunk(num_blocks)
    ...
    for i, samples in enumerate(samples_list):
        ...
        samples.info["acc_reward"] = acc_list[i]
        samples.info["format_reward"] = format_list[i]
        if "extra_logs" in rewards_info[0]:
...
class RemoteExperienceMaker(ABC):
    ...
    def make_experience(self, samples_list: List[Experience]) -> List[Experience]:
        ...
        # prompt2id = {}
        # id_list = []
        # cur_id = 1  # 从1开始编号，也可以从0
        #
        # for p in prompts:
        #     if p not in prompt2id:
        #         prompt2id[p] = cur_id
        #         cur_id += 1
        #     id_list.append(prompt2id[p])
        ...
        if samples_list[0].rewards is not None:
            ...
            r_refs = self.remote_reward_model.get_rewards.remote(queries_list, prompts_list, labels_list, self.tokenizer)
        else:
